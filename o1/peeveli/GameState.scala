...
class GameState(val missesAllowed: Int, val previousGuesses: String, val visibleWord: String, val possibleSolutions: Vector[String])  {
  ...
  private def reveal(patternToReveal: String) = {
    visibleWord.indices.map(i => if(visibleWord(i) != '_' || patternToReveal(i) == visibleWord(i)) visibleWord(i)
    else patternToReveal(i)).mkString("")
  }
  ...
  def guessLetter(guess: Char) = {
    ...
    var misses = missesAllowed - 1
    var oneMoreRevealed = visibleWord
    val solutionHelp = this.possibleSolutions.groupBy(_.map(x => if(x != actualGuess){'_'} else {actualGuess})).maxBy(_._2.size)
    val solution = solutionHelp._2
    if(solutionHelp._1.count(_ == actualGuess) != 0){
      misses += 1
      oneMoreRevealed = reveal(solutionHelp._1)
    }
    new GameState(misses, this.previousGuesses + actualGuess, oneMoreRevealed, solution)
  }
